...
### 本地路径与 OpenList 网盘路径后台导入

本地路径和 OpenList 网盘路径不是 TXT 目录树导入：选定路径后会直接创建可恢复的后台批次，依次完成作品识别、媒体库镜像生成和资料补充。
...
本地路径 / OpenList 网盘路径：选择路径 → 后台识别 → 生成媒体库 → 自动补充资料
```

- TXT 目录树可保存为 115 / 百度媒体库预设，之后上传新版 TXT 做增量对比。
- 本地路径与 OpenList 网盘路径会在后台持续处理，并在观察页显示作品级进度、完成结果与待处理项目。
