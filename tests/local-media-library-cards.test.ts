...
test('本地媒体库卡片按实际路径展示并使用重新扫描操作', () => {
  ...
  assert.match(page, /sourcesApi\.scanLocal\(/);
  assert.match(page, /importsApi\.getPreview\('local', result\.plan_id\)/);
  assert.match(page, /setStep\('confirm'\)/);
  assert.doesNotMatch(page, /sourcesApi\.createLocalImportBatch\(/);
