...
import {
  ...
  type ScrapeTarget,
} from '../api/workDetailV4Compatibility';
...
export default function WorkDetailPage() {
  ...
  useEffect(() => {
    if (!workDetailV4Capabilities.appendEpisodes || !work || work.media_type !== 'tv') return;
    let disposed = false;
  ...
  const refreshPlaybackSnapshot = async () => {
    ...
    if (historyResult.status === 'fulfilled') {
      ...
      const latestHistoryId = payload.items?.[0]?.updated_at || '';
      const globalLatestHistoryId = useLibraryStore.getState().history[0]?.updated_at || '';
  ...
  const hasDetailTags = workSources.length > 0 || titleTags.length > 0 || (isSeries && seasons.length > 0) || (!isSeries && !!work.year) || work.rating > 0 || (workDetailV4Capabilities.bangumiBinding && !!bangumiStatusLabel);
    ...
    >
      ...
      <section className="detail-hero-copy" aria-label="作品概要" style={{ pointerEvents: 'auto' }}>
        ...
        {hasDetailTags && (
          <div className="detail-hero-tags">
            ...
            {workDetailV4Capabilities.bangumiBinding && <button
        ...
        <div className="detail-hero-actions">
          ...
          <div className="detail-more-wrap" ref={moreMenuRef}>
            ...
            {moreMenuOpen && <div className="detail-more-menu" role="menu">
              {workDetailV4Capabilities.titleMutation && <button role="menuitem" onClick={openTitleEditor}><Pencil size={16} />修改作品标题</button>}
              ...
              {workDetailV4Capabilities.manualScrape && <button role="menuitem" onClick={() => {
                setScrapeScope('work');
      ...
      <div className="detail-main-grid">
        <section className="detail-info min-w-0 space-y-5">
          {workDetailV4Capabilities.manualScrape && manualScrapeOpen && createPortal(
          ...
          {workDetailV4Capabilities.bangumiBinding && bangumiPanelOpen && createPortal(
      ...
      {workDetailV4Capabilities.artworkMutation && artworkKind && createPortal(
      ...
      {workDetailV4Capabilities.titleMutation && titleEditOpen && createPortal(
      ...
      {workDetailV4Capabilities.workDeletion && deleteWorkOpen && createPortal(
      ...
      {workDetailV4Capabilities.appendEpisodes && appendOpen && createPortal(
        <div className="seasonal-dialog-backdrop" role="presentation">
